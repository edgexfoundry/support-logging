...
public class LoggingControllerTest {
	...
	@Autowired @Qualifier("serviceDAO")
	...
	public void cleanup() throws Exception {
		mongoTemplate.remove(new Query(), "logEntry");
		//TODO changeControllerService(true);
	}

	@Test
	public void testAddLogEntry() throws InterruptedException {		
		LogEntry entry = buildLogEntry("testService", Level.DEBUG, new String[]{"testLabel1", "testLabel2"}, "test log message from testcase.");
		verifyAddLogEntry(entry);
	}
	
	//TODO @Test
	public void testGetLogEntries() throws InterruptedException {		
		LogEntry debug = buildLogEntry("debugService", Level.DEBUG, new String[]{"debug1", "debug2"}, "test log message for debug.");
		LogEntry info = buildLogEntry("infoService", Level.INFO, new String[]{"info1", "info2", "info3"}, "test log message for info.");
		LogEntry warn = buildLogEntry("warnService", Level.WARN, new String[]{"warn1"}, "test log message for warn.");
		LogEntry error = buildLogEntry("errorService", Level.ERROR, new String[]{"error1", "error2", "error3", "error4"}, "test log message for error.");
		LogEntry trace = buildLogEntry("traceService", Level.TRACE, new String[]{"trace1", "trace2", "trace3", "trace4", "trace5"}, "test log message for trace.");
		verifyAddLogEntry(debug);
		verifyAddLogEntry(info);
		verifyAddLogEntry(warn);
		verifyAddLogEntry(error);
		verifyAddLogEntry(trace);
		List<LogEntry> logEntries = controller.getLogEntries(10);
		assertTrue("Expect 5 but got " + logEntries.size() + " logEntries.", logEntries.size()==5);
	}
	...
	private void verifyAddLogEntry(LogEntry entry) throws InterruptedException{
		...
		Thread.sleep(3000);
		if (loggingPersistence.equals("mongodb")){
			verifyMongoPersistence(entry);
		}
	}
	
	//TODO @Test(expected = ServiceException.class)
	...
	private LogEntry buildLogEntry(String originService, Level LogLevel, String[] labels, String message){
		LogEntry entry = new LogEntry();
	...
	private void verifyMongoPersistence(LogEntry entry){
		List<LogEntry> result = mongoTemplate.find(new Query().addCriteria(buildCriteria(entry)), LogEntry.class);
		assertTrue("Expect to find one logEntry in MongoDB but none.", result.size()==1);
	}
	
	private Criteria buildCriteria(LogEntry entry){
		Criteria result = Criteria.where(MDC_ENUM_CONSTANTS.ORIGINSERVICE.getValue()).is(entry.getOriginService())
				.and(MDC_ENUM_CONSTANTS.LOGLEVEL.getValue()).is(entry.getLogLevel())
				.and(MDC_ENUM_CONSTANTS.MESSAGE.getValue()).is(entry.getMessage())
				.and(MDC_ENUM_CONSTANTS.LABELS.getValue()).in((Object[])entry.getLabels());
		return result;
	}
	
	// use Java reflection to change service's DAO
	/*private void changeServiceDao(boolean isReset) throws Exception {
		Class<?> serviceClass = service.getClass();
		Field temp = serviceClass.getDeclaredField("logEntryDAO");
		temp.setAccessible(true);
		if(isReset){
			temp.set(service, logEntryDAO);
		} else {
